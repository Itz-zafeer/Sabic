        ...
        {formLinks.map((formLink, i) => (
          <li key={i}>
            <Link
              ...
              href={`${formLink.link}`}
              className="opacity_Hover flex gap-x-[11px] lg:gap-x-[0.76388888888vw] items-center"
            >
              <img
                src={formLink.img}
                alt={`${formLink.alt}`}
                className="lg:w-[1.28958333333vw] w-[18.57px] flipped"
              />
              <span className="text-white text-opacity-[0.7] underline">
                {" "}
                {formLink.text}
              </span>
            </Link>
